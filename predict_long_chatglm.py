...
from Prompts_openai import meta_prompt, scorer_prompt, prompts
import pandas as pd
import os
...
def save_sample_answers_with_index_to_excel(sample_answers_with_index, file_count):
    ...
    # 将数据保存到Excel文件
    excel_file_name = f"/root/autodl-tmp/LLM as Optimizer/llm_as_optimizer/Optimization-by-PROmpting/checkpoint/sample_answers_batch_{file_count}.xlsx"
...
def score_evaluates(prompts, training_examples, performance_df):
    ...
    for prompt in prompts:
        ...
        for _, example in training_examples.iterrows():
            ...
            print(sample_answer)
            predictions.append(1 if '1' in sample_answer else 0)
            labels.append(label)

            # 将sample_answer和索引添加到列表中
            sample_answers_with_index.append((index, sample_answer))

            # 当sample_answers_with_index的长度达到100的倍数时，保存到Excel文件并清空列表
            if len(sample_answers_with_index) % 100 == 0:
        ...
        # save the checkpoint
        performance_df.to_excel("/root/autodl-tmp/LLM as Optimizer/llm_as_optimizer/Optimization-by-PROmpting/checkpoint/performance_dm2_structure.xlsx", index=False)
...
if  __name__ == "__main__":
    ...
    training_data_df = pd.read_excel("/root/autodl-tmp/LLM as Optimizer/llm_as_optimizer/Optimization-by-PROmpting/data/training_data.xlsx").iloc[100:1000]
    evaluation_df = predict_eval(performance_df, training_data_df, prompts)
    print(evaluation_df)
    evaluation_df.to_excel("/root/autodl-tmp/LLM as Optimizer/llm_as_optimizer/Optimization-by-PROmpting/data/performance_predict_final_dm2_structure.xlsx", index=False)
